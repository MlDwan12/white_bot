...
MAX. Личный токен VK для вложений живёт сутки и продлевается вручную — карточка
на странице «Группы» (два шага, статус виден сразу).
...
**Уже сделано для деплоя:** в стадии `production` есть `prisma/` и
`prisma.config.ts`, `prisma` объявлен в `dependencies` (миграции запускаются из
самого образа), `storage` и `PROGRESS.md` не попадают в контекст сборки.
Осталось подтвердить это реальной сборкой образа.
